...
import * as Convert from "@truffle/codec/abi-data/convert";
...
function allocateCalldata(
  ...
  if (allocationMode === "full") {
    ...
    parameterTypes = parameters.map(parameter => ({
      name: parameter.name,
      type: Ast.Convert.definitionToType(parameter, compiler) //if node is defined, compiler had also better be!
    }));
  ...
  if (allocationMode === "abi") {
    ...
    parameterTypes = abiEntry.inputs.map(parameter => ({
      name: parameter.name,
      type: Convert.abiParameterToType(parameter)
    }));
    abiAllocation = allocateMembers(
      id,
      parameterTypes,
...
function allocateEvent(
  ...
  if (allocationMode === "full") {
    ...
    parameterTypes = parameters.map(definition => ({
      ...
      type: Ast.Convert.definitionToType(definition, compiler),
  ...
  if (allocationMode === "abi") {
    ...
    parameterTypes = abiEntry.inputs.map(abiParameter => ({
      type: Convert.abiParameterToType(abiParameter),
