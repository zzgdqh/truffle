...
export class WireDecoder {
  ...
  private collectUserDefinedTypes(): {
    ...
    for (const id in this.contracts) {
      ...
      types[id] = Ast.Convert.definitionToStoredType(contractNode, compiler);
      ...
      for (const node of contractNode.nodes) {
        if (
          ...
          types[node.id] = Ast.Convert.definitionToStoredType(
            node,
            compiler,
            references
          );
...
export class ContractInstanceDecoder {
  ...
  public async state(
    ...
    return {
      class: Contexts.Convert.contextToType(this.context),
